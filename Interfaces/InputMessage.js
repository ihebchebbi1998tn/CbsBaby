...
const InputMessage = ({ sessionId, recieverID }) => {
  ...
  const [languageModalVisible, setLanguageModalVisible] = useState(false); // State for language selection modal visibility
  const { enableTranslationsChat, toggleTranslations } = useEnableTranslation();
  ...
  const handleSendMessage = async () => {
    ...
    let url = `${BASE_URL}bebeapp/api/Messaging/send_message.php?session_id=${sessionId}&sender_id=${SENDER_ID}&receiver_id=${recieverID}&type=text&message_text=${encodeURIComponent(
      message
    )}&message_time=${encodeURIComponent(
      messageTime
    )}&profile_image=https://cdn-icons-png.flaticon.com/512/4478/4478097.png&messages_bot=0`;

    try {
      const response = await fetch(url);
      const data = await response.json();
      sendNotification(nurseTokens, message);
  ...
  const handleSendImage = async () => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}bebeapp/api/Messaging/send_photo.php`,
        {
          method: "POST",
          body: formData,
          headers: {
  ...
  const sendPredefinedMessages = async () => {
    const welcomeMessage = "Bienvenue ,"+ user.name +  "🎉";
    const introMessage =
      "Je suis votre assistante à la clinique Beau Séjour. Comment puis-je vous aider ? 😊";
    const messageTime = new Date().toISOString();

    try {
      let url = `${BASE_URL}bebeapp/api/Messaging/send_message.php?session_id=${sessionId}&sender_id=123456789&receiver_id=${SENDER_ID}&type=text&message_text=${encodeURIComponent(
        welcomeMessage
      )}&message_time=${encodeURIComponent(
        messageTime
      )}&profile_image=https://t4.ftcdn.net/jpg/01/85/46/17/360_F_185461775_hOtNw5K9ajrEBG8Wxtj6RWFqX1TgewAC.jpg&messages_bot=1`;
      let response = await fetch(url);
      let data = await response.json();
      await new Promise((resolve) => setTimeout(resolve, 4000)); // Delay for 1 second
      url = `${BASE_URL}bebeapp/api/Messaging/send_message.php?session_id=${sessionId}&sender_id=123456789&receiver_id=${SENDER_ID}&type=text&message_text=${encodeURIComponent(
        introMessage
      )}&message_time=${encodeURIComponent(
        messageTime
      )}&profile_image=https://t4.ftcdn.net/jpg/01/85/46/17/360_F_185461775_hOtNw5K9ajrEBG8Wxtj6RWFqX1TgewAC.jpg&messages_bot=1`;
  ...
  const checkAndSendPredefinedMessages = async () => {
    try {
      const response = await fetch(
        `${BASE_URL}bebeapp/api/Messaging/check_first_user_message.php?session_id=${sessionId}`
      );
      const data = await response.json();
      console.log(data);
  ...
  const sendResponseMessage = async () => {
    ...
    const messageTime = new Date().toISOString();
    try {
      const url = `${BASE_URL}bebeapp/api/Messaging/send_message.php?session_id=${sessionId}&sender_id=123456789&receiver_id=${SENDER_ID}&type=text&message_text=${encodeURIComponent(
        responseMessage
      )}&message_time=${encodeURIComponent(
        messageTime
      )}&profile_image=https://t4.ftcdn.net/jpg/01/85/46/17/360_F_185461775_hOtNw5K9ajrEBG8Wxtj6RWFqX1TgewAC.jpg&messages_bot=1`;
  ...
  const fetchNurseTokens = async () => {
    try {
      const response = await fetch(
        `${BASE_URL}bebeapp/api/Messaging/get_nurses_token.php`
      );
      const data = await response.json();
      if (data.success) {
  ...
  return (
    <View style={styles.inputContainer}>
      ...
      >
        <Ionicons name="language-outline" size={24} color="#D84374" />
      </TouchableOpacity>
      <TouchableOpacity
        ...
        <View style={styles.languageModalContainer}>
          <View style={styles.languageModal}>
            <TouchableOpacity
              ...
              style={styles.closeButton}
            >
              <Ionicons name="close" size={24} color="#D84374" />
            </TouchableOpacity>
           {/* s */}
            {/* <TouchableOpacity
            ...
            <View style={styles.translationSwitchContainer}>
              <Text style={styles.translationText}>Translations:</Text>
              <Switch
                trackColor={{ false: "#767577", true: "#81b0ff" }}
                thumbColor={enableTranslationsChat ? "#D84374" : "#f4f3f4"}
                ios_backgroundColor="#3e3e3e"
                onValueChange={toggleTranslations}
                value={enableTranslationsChat} // Bind value to enableTranslationsChat state
              />
            </View>
          </View>
        </View>
